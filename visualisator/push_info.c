...
void	push_winner_vis(void)
{
	...
	SDL_DestroyTexture(text_texture);
	pos.x += pos.w + 10;
	text_surface = TTF_RenderText_Solid(g_font, g_players[g_vm->last_live_player].name, winner);
	text_texture = SDL_CreateTextureFromSurface(g_main_render, text_surface);
	SDL_FreeSurface(text_surface);
	TTF_SizeText(g_font, g_players[g_vm->last_live_player].name, &pos.w, &pos.h);
	SDL_RenderCopy(g_main_render, text_texture, NULL, &pos);
	SDL_DestroyTexture(text_texture);
}
